...
SECRET_KEY = "X0P2OjPzQtzm2MNmRzuaOSmUZ_u-l6ljGE9zciH1Nm"
ALGORITHM = "HS256"
...
def register_user(user: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    # Return the value (not including the password)
    return {
        "id": new_user.id,
...
def require_roles(allowed_roles: list[str]):
    def check_roles(current_user: dict = Depends(get_current_user)):
        ...
        if user_role not in allowed_roles:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="No permission granted"
            )
